...
from Mikobot import (BOT_NAME, BOT_USERNAME, LOG_GROUP_ID,
                 aiohttpsession, app, app2)
from Mikobot.modules import ALL_MODULES
from Mikobot.adv import ADV_MODULES
from Mikobot.base import BASE_MODULES
from Mikobot.fun import FUN_MODULES
from Mikobot.utils import paginate_modules
from Mikobot.utils.dbfunctions import clean_restart_stage
from Mikobot import LOGGER
from Mikobot.utils.dbfunctions import  get_served_chats, get_served_users
from Mikobot.utils.dbfunctions import get_gbans_count,get_served_chats,get_served_users                    
import pytz
import datetime
...
async def start_bot():
    ...
    for module_name in ALL_MODULES:
        imported_module = importlib.import_module("Mikobot.modules." + module_name)
...
#new end   
    for module_name in ADV_MODULES:
        imported_adv = importlib.import_module("Mikobot.adv." + module_name)
        # imported_adv = importlib.import_module("Mikobot.adv." + module_name)
    ...
    for module_name in BASE_MODULES:
        imported_base = importlib.import_module("Mikobot.base." + module_name)
        # imported_base = importlib.import_module("Mikobot.base." + module_name)
    ...
    for module_name in FUN_MODULES:
        imported_fun = importlib.import_module("Mikobot.fun." + module_name)
        # imported_fun = importlib.import_module("Mikobot.fun." + module_name)
...
✘ Base commands are the basic tools of Mikobot Bot which help you to manage 
...
started time - `2022 {dater}:{timer} `         
        ...
        return await message.reply(
            "**Heya, @szMikobotbot here :) PM me if you have any questions how to use me!**", reply_markup=keyboard
        )
...
Mikobot supports a large number of formatting options 
...
Use this to make Mikobot feel a bit more customised! (only works in filters/notes)
...
async def commands_callbacc(_, CallbackQuery):
    ...
    TEXT_PART = f"""
@szMikobotbot ** is one of the fastest and most feature filled group manager**.   
 
 - **Latest Update Time** : `2022 {dater}:{timer}`
 - **Modules Loaded **:  `{modules_count}`
 - **Globally banned users.** :  `{gbans}`
@szMikobotbot **Mongo database stats**
 - ** Users ** : `{served_users}`
 - ** chats.** : `{served_chats}`
...
TEXT_PART_1 = """
**Welcome to the Mikobot Configuration**
The first thing to do is to add Mikobot Bot ✨ to your group! 
...
BUTTON_1 = InlineKeyboardMarkup(
        [[        
        InlineKeyboardButton('Add Mikobot to Chat ! 🎉', url='http://t.me/szMikobotbot?startgroup=new')
        ],
...
Mikobot ✨ is developed and actively maintained by @szteambots!

Mikobot has been online since 2021/8/10 and have many [groups and users.](https://t.me/szteambots/890)
